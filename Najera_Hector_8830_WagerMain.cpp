...
int main()
{
    ...
    if (no <= 0)
    {
	cout << "Nobody playing?! ... Good Bye ..." << endl << endl;
	return 0;
    }
    string name;
    for(int i = 1; i <= no; i++)
    ...
    cout << "Each player starts with $10 in the bank." << endl << endl;
    ...
    {
	round++;
	cout << "Round" << round << ": Your wagers ..." << endl << endl;
	...
	if(!players_on.empty())
	{
	    ...
	    cout << "PLAYERS ON TARGET (" << target << ") WIN SHARE OF THE POT..." << endl;

	    for(int i = 0; i < players_on.size(); i++)
	    {
		bank[players_on[i]] += share;
		cout << players_on[i] << "... wins $" << share << endl;
	    }
	    pot = 0;
	}
	else if(!players_above.empty())
	{
	    ...
	    cout << "None on target (" << target << "), but YOU ARE ABOVE..." << endl;
	    for(int i = 0; i < players_above.size(); i++)
	    {
		bank[players_above[i]] += share;
		cout << players_above[i] << "... wins $" << share << endl;
	    }
	    pot = pot / 2.0;
	}
	else
	    cout << "Sorry, all players lose this round" << endl;
    ...
    cout << "GAME OVER... THIS IS HOW YOU FARED:" << endl << endl;
    ...
    {
	cout << "Player " << *itr << " goes home with $" << bank[*itr];
	if(bank[*itr] < 0)
	    cout << " ...please pay the cashier on the way out" << endl;
	if(bank[*itr] > 0)
	    cout << " ...your voucher for next time will be in the mail" << endl;
    }
    cout << endl << endl;
    cout << "Come back soon. Play responsibly ..." << endl << endl;
